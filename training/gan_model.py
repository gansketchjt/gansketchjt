...
class GANModel(jt.Module):
    ...
    def create_optimizers(self, opt):
        ...
        d_reg_ratio = ((opt.d_reg_every / (opt.d_reg_every + 1)) if (not opt.no_d_regularize) else 1.0)
    ...
    def create_loss_fns(self, opt):
        self.criterionGAN = networks.GANLoss(opt.gan_mode, tensor=jt.float32, opt=self.opt)
        if (opt.l_weight > 0):
    ...
    def load(self, iters):
        ...
        self.netG.load_parameters(state_dict_g)
        state_dict_d_sketch = jt.load(load_path + 'D_sketch.pth')
        self.netD_sketch.load_parameters(state_dict_d_sketch)
        if (self.opt.l_image > 0):
            state_dict_d_image = jt.load(load_path + 'D_image.pth')
            self.netD_image.load_parameters(state_dict_d_image)
    ...
    def initialize_networks(self, opt):
        ...
        if ((netD_sketch is not None) and (opt.d_pretrained != '') and (not opt.dsketch_no_pretrain)):
            ...
            netD_sketch.load_parameters(weights)
        if (opt.l_image > 0):
            ...
            if (opt.d_pretrained != ''):
                ...
                weights = jt.load(opt.d_pretrained)
                netD_image.load_parameters(weights)
    ...
    def compute_generator_loss(self):
        ...
        G_losses['G_sketch'] = self.criterionGAN(pred_fake, True, for_discriminator=False)
        if (self.opt.l_image > 0):
            pred_fake2 = self.netD_image(fake_image)
            G_losses['G_image'] = (self.opt.l_image * self.criterionGAN(pred_fake2, True, for_discriminator=False))
    ...
    def compute_discriminator_loss(self, real_sketch, real_image):
        ...
        D_losses['D_fake_sketch'] = self.criterionGAN(pred_fake, False, for_discriminator=True)
        D_losses['D_real_sketch'] = self.criterionGAN(pred_real, True, for_discriminator=True)
        if (self.opt.l_image > 0):
            pred_fake2 = self.netD_image(fake_image)
            pred_real2 = self.netD_image(real_image)
            D_losses['D_fake_image'] = (self.opt.l_image * self.criterionGAN(pred_fake2, False, for_discriminator=True))
            D_losses['D_real_image'] = (self.opt.l_image * self.criterionGAN(pred_real2, True, for_discriminator=True))
        if self.opt.reduce_visuals:
            interm_imgs = {}
        else:
            interm_imgs = {'real_sketch': real_sketch.detach(), 'fake_image': fake_image.detach(), 'real_inputD': real_transf.detach(), 'fake_inputD': fake_transf.detach()}
    ...
    def compute_discriminator_regularization(self, real_sketch, real_image):
        ...
        if (self.opt.l_image > 0):
            ...
            D_reg_losses['D_r1_loss_image'] = ((((self.opt.l_image * self.opt.r1) / 2) * r1_loss2) * self.opt.d_reg_every)
        return D_reg_losses
