...
def create_dataloader(data_dir, size, batch, img_channel=3):
    ...
    transform = transforms.Compose([transforms.Resize(size), transforms.ToTensor(), transforms.ImageNormalize(mean, std)])
    if (img_channel == 1):
        image_mode = 'L'
    elif (img_channel == 3):
        image_mode = 'RGB'
    ...
    loader=dataset.set_attrs(batch_size=batch, drop_last=True)
