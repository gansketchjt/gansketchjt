...
import jittor as jt
from jittor import float32, init, size, std
from jittor import nn
import math
import random
from .op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
import jittor.compile_extern
import numpy as np
if jittor.compile_extern.has_cuda:
...
class PixelNorm(nn.Module):
    ...
    def execute(self, input):
        return (input * 1.0/jt.sqrt((jt.mean((input ** 2), dim=1, keepdims=True) + 1e-08)))

def make_kernel(k):
    k=jt.Var(k).astype(jt.float32)
    if (k.ndim == 1):
        k = (k[None, :] * k[:, None])
    k /= k.sum()
    return k
...
class EqualLinear(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, bias=True, bias_init=0, lr_mul=1, activation=None):
        super().__init__()
        self.weight = jt.array(jt.randn(out_dim, in_dim).divide(lr_mul)) # changed from div_ to divice
        if bias:
            self.bias = jt.array(jt.full(out_dim, bias_init))
        else:
            self.bias = None
    ...
    def execute(self, input):
        ...
        if self.activation:
            ...
            out = nn.matmul_transpose(input, self.weight * self.scale) + self.bias * self.lr_mul
        return out
...
class ModulatedConv2d(nn.Module):
    ...
    def execute(self, input, style):
        ...
        if self.demodulate:
            demod = 1.0/jt.sqrt((weight.pow(2).sum([2, 3, 4]) + 1e-08))
        ...
        if self.upsample:
            ...
            height = (height - 1) * 2 + self.kernel_size
            width = (width - 1) * 2 + self.kernel_size
            out = jt.cudnn.ops.cudnn_conv_backward_x(weight, input, height=height, width=width, strideh=2, stridew=2, paddingh=0, paddingw=0, dilationh=1, dilationw=1, groups=batch)
...
class NoiseInjection(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.weight = jt.array(jt.zeros(1))

    def execute(self, image, noise=None):
     
        if (noise is None):
            (batch, _, height, width) = image.shape
            noise = jt.normal(mean=0, std=1, size=(batch, 1, height, width)).type_as(image)
...
class Generator(nn.Module):
    ...
    def __init__(self, size, style_dim, n_mlp, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, w_shift=False):
        ...
        for layer_idx in range(self.num_layers):
            ...
            setattr(self.noises, f'_noise_{layer_idx}', jt.randn(*shape))
    ...
    def execute(self, styles, return_latents=False, inject_index=None, truncation=1, truncation_latent=None, input_is_latent=False, noise=None, randomize_noise=True):
        ...
        if (noise is None):
            if randomize_noise:
                ...
                noise = [getattr(self.noises, f'_noise_{i}') for i in range(self.num_layers)]
        if (truncation < 1):
            style_t = []
            for style in styles:
        ...
        i = 1
        for (conv1, conv2, noise1, noise2, to_rgb) in zip(list(self.convs.values())[::2], list(self.convs.values())[1::2], noise[1::2], noise[2::2], self.to_rgbs):
...
class Discriminator(nn.Module):
    ...
    def execute(self, input):
        ...
        stddev=jt.sqrt(jt.sqr(stddev-stddev.mean(dim=0)).sum(dim=0)/stddev.shape[1]+1e-08)
