    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            if (self.real_label_tensor is None):
                self.real_label_tensor = self.Tensor(jt.Var(self.real_label))
                self.real_label_tensor.requires_grad_(False)
            return self.real_label_tensor.expand_as(input)
        else:
            if (self.fake_label_tensor is None):
                self.fake_label_tensor = self.Tensor(jt.Var(self.fake_label))
                self.fake_label_tensor.requires_grad_(False)
            return self.fake_label_tensor.expand_as(input)

    def get_zero_tensor(self, input):
        if (self.zero_tensor is None):
            self.zero_tensor = self.Tensor(jt.Var(0))
    ...
    def loss(self, input, target_is_real, for_discriminator=True):
        if (self.gan_mode == 'original'):
            ...
            if for_discriminator:
                if target_is_real:
                    minval = jt.min(
                        (input - 1), self.get_zero_tensor(input))
                    loss = (- jt.mean(minval))
                else:
                    minval = jt.min(
                        ((- input) - 1), self.get_zero_tensor(input))
    ...
    def execute(self, input, target_is_real, for_discriminator=True):
        if isinstance(input, list):
            ...
            for pred_i in input:
                ...
                bs = (1 if (len(loss_tensor.shape) == 0)
                      else loss_tensor.shape[0])
...
class WeightLoss(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.ref_weights = [jt.Var(
            p).stop_grad() for p in params]
