...
def compute_ppl(g, num_samples=50000, epsilon=0.0001, space='w', sampling='end', crop=False, batch_size=25):
    ...
    with jt.no_grad():
        for batch in tqdm(batch_sizes):
            ...
            if (factor > 1):
                image = jt.nn.interpolate(image, size=(256, 256), mode='bilinear', align_corners=False)
            dist = (percept(torch.from_numpy( np.array(image[::2])),torch.from_numpy( np.array(image[1::2]))).view((image.shape[0] // 2)) / (epsilon ** 2))
            distances.append(dist.to('cpu').detach().numpy())
